...
export const enUS: Record<TranslationKey, string> = {
  ...
  "app.composerAddTitle": "Add an attachment, skill, or prompt",
  ...
  "app.skillPickerNoProject": "This session has no project, so project skills cannot be read",
  ...
  "config.dsh.ungroupedAdopt": "dsh-web Ungrouped",
  "config.dsh.ungroupedAdoptEmpty": "No adoptable ungrouped root sessions. Subagents and directories without a workspace stay hidden here.",
  "config.dsh.ungroupedAdoptHint": "Only root sessions whose cwd already matches an existing workspace are adopted, via official sessions.create({workspaceId, sessionId}). Workspace.json is never written by hand. Subagents and directories without a workspace stay Ungrouped. Close dsh-web first — two hosts cannot share one DSH_HOME.",
  "config.dsh.ungroupedAdoptAll": "Adopt into matching workspace",
  "config.dsh.ungroupedAdopted": "Adopted {count} ungrouped session(s).",
  "config.dsh.ungroupedAdoptPartial": "Adopt finished: {adopted} ok, {failed} failed.",
  "config.dsh.projectionTitles": "dsh-web session titles",
  "config.dsh.projectionTitlesHint": "dsh-web's cold list reads only the official projection cache. Older PiDeck sessions often have titles only in the log, so a missing cache row shows the directory name. Backfill uses official coldSnapshot and covers historical sessions. Cache JSON is never written by hand. Close dsh-web first — two hosts cannot share one DSH_HOME.",
  "config.dsh.projectionTitlesEmpty": "Every root session in the projection cache already has a title.",
  "config.dsh.projectionTitlesMissing": "{missing} root session(s) are missing a cached title (dsh-web will show the directory name). {titled} already have titles.",
  "config.dsh.projectionTitlesBackfill": "Backfill historical titles",
  "config.dsh.projectionTitlesDone": "Backfilled {attempted} session title(s). Stop the PiDeck host, then reopen dsh-web to see them.",
  "config.dsh.projectionTitlesPartial": "Backfill finished: tried {attempted}, failed {failed}.",
  ...
  "config.dsh.customSettingsHint": "Set the API base URL, protocol, and display name here. Put the API key in the field above — not in the env-var name.",
  ...
  "config.dsh.rawHint": "Edit YAML files under {dir} directly; saved changes are picked up by the DSH host.",
  ...
  "config.dsh.tab.presets": "Presets",
  ...
  "config.imagegen.sectionDesc": "Separate from the pi models page. Requests always use the OpenAI-compatible /images/generations API; Volcengine only needs a different base URL. Enable the official fields this provider actually supports — only then do they appear in the composer.",
...
"config.emptyExtensions": "No installed extensions.",
  ...
  "imagegen.error.notConfigured": "No image provider is configured. Open Pi management → Agent capabilities → Image Gen and add a base URL, API key, and model.",
  ...
  "imagegen.notConfiguredHint": "Add a provider under Pi management → Image Gen",
  ...
  "settings.piProxyProviders": "Proxy by provider",
  "settings.piProxyProvidersDesc": "When the allowlist is non-empty, only listed providers are forced through the proxy (reusing the address above even if the global switch is off) and others are forced direct. Leave empty to disable provider filtering (follow global / per-session settings). Fixes the \u201Cfirst request without proxy before activating session\u201D issue.",
  "settings.piProxyProvidersHint": "Changes apply to new sessions immediately by provider. Active sessions need a restart. Per-session \u201CSession proxy\u201D overrides still have highest priority.",
  "settings.piProxyProvidersEmpty": "No providers available (configure providers in Models or refresh)",
  "settings.piProxyProvidersLoading": "Loading providers…",
  "settings.piProxyProvidersSelected": "{count} provider(s) selected for proxy",
  "settings.piProxyProvidersAllFollow": "No allowlist: all follow global / per-session settings",
  "settings.piProxyProvidersClear": "Clear",
