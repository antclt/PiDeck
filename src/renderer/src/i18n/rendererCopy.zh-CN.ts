...
export const zhCN = {
  ...
  // ── 加号菜单（附件/技能/Prompt 三合一）──
  "app.composerAddTitle": "添加附件、技能或 Prompt",
  ...
  "app.skillPickerNoProject": "当前会话无项目，无法读取项目技能",
  ...
  "config.dsh.ungroupedAdopt": "dsh-web 未分组",
  "config.dsh.ungroupedAdoptEmpty": "没有可认领的未分组根会话。子代理、以及目录还没有 workspace 的会话不会出现在这里。",
  "config.dsh.ungroupedAdoptHint": "只认领工作目录已经对应现有 workspace 的根会话，走官方 sessions.create({workspaceId, sessionId})，不手写 workspace.json。子代理和没有 workspace 的目录保持未分组。请先关掉 dsh-web（同一 DSH_HOME 不能双 host）。",
  "config.dsh.ungroupedAdoptAll": "认领到对应分组",
  "config.dsh.ungroupedAdopted": "已认领 {count} 个未分组会话。",
  "config.dsh.ungroupedAdoptPartial": "认领完成：成功 {adopted}，失败 {failed}。",
  "config.dsh.projectionTitles": "dsh-web 会话标题",
  "config.dsh.projectionTitlesHint": "dsh-web 冷列表只读官方投影缓存。旧 PiDeck 会话的标题往往只在日志里，缓存缺行就会显示成目录名。回写走官方 coldSnapshot，覆盖历史会话，不手写缓存。请先关掉 dsh-web（同一 DSH_HOME 不能双 host）。",
  "config.dsh.projectionTitlesEmpty": "投影缓存里的根会话都已有标题。",
  "config.dsh.projectionTitlesMissing": "{missing} 个根会话的缓存缺标题（dsh-web 会显示成目录名）。已有标题 {titled} 个。",
  "config.dsh.projectionTitlesBackfill": "回写历史标题",
  "config.dsh.projectionTitlesDone": "已回写 {attempted} 个会话标题。关掉 PiDeck host 后再开 dsh-web 即可看到。",
  "config.dsh.projectionTitlesPartial": "回写完成：尝试 {attempted}，失败 {failed}。",
  ...
  "config.dsh.customSettingsHint": "这里改接口地址、协议和显示名。API 密钥请填在上方单独的密钥框，不要填到环境变量名里。",
  ...
  "config.dsh.rawHint": "直接编辑 {dir} 下的 YAML；保存后由 DSH host 读取（改动范围以官方文档为准）。",
  ...
  "config.dsh.pluginAgentLoop": "Agent Loop（智能体循环）",
  "config.dsh.pluginShell": "Shell（命令执行）",
  "config.dsh.pluginWebSearch": "Web Search（网络搜索）",
  ...
  "config.imagegen.sectionDesc": "独立于 pi 模型页。接口一律按 OpenAI 兼容 /images/generations 发送；火山等只需换 baseUrl。勾选该供应商实际支持的官方字段后，会话生图底栏才会出现对应输入。",
...
"config.emptyExtensions": "暂无已安装扩展。",
  ...
  "imagegen.error.notConfigured": "还没有配好生图供应商。请到 Pi 管理 → Agent 能力 → 生图 填写接口地址、API Key 和模型",
  ...
  "imagegen.notConfiguredHint": "先到 Pi 管理 → 生图 添加供应商",
  ...
  "settings.piProxyProviders": "按供应商走代理",
  "settings.piProxyProvidersDesc": "仅名单内供应商强制走代理（即使全局关闭也复用上方地址），名单外强制直连。留空则不按供应商过滤（沿用全局/会话设置），可解决“新建会话首条请求无代理”的问题",
  "settings.piProxyProvidersHint": "改动后新建会话即按供应商自动匹配代理；已激活会话需重启生效；会话级“会话代理”手动指定仍最高优",
  "settings.piProxyProvidersEmpty": "暂无可选供应商（请先在模型配置中配置供应商，或刷新后重试）",
  "settings.piProxyProvidersLoading": "加载供应商中…",
  "settings.piProxyProvidersSelected": "已选 {count} 个供应商走代理",
  "settings.piProxyProvidersAllFollow": "未配置白名单：全部跟随全局/会话设置",
  "settings.piProxyProvidersClear": "清空",
