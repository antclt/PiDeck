...
export const zhCN = {
  ...
  "config.dsh.orphans": "{count} 个外部 DSH 会话（dsh-web 等其他工具创建，可导入 PiDeck 使用）",
  "config.dsh.foreignSessions": "外部会话（其他工具）",
  "config.dsh.foreignSessionsEmpty": "暂无外部会话。",
  "config.dsh.foreignSessionsHint":
    "由 dsh-web 等其他工具在 DSH_HOME 里创建的会话，PiDeck 尚未建立映射。导入后可在侧栏查看、加载历史并继续对话（目标项目按目录自动匹配）。",
  ...
  "config.dsh.staticPluginsHint": "当前部署 cordis 组合中的 Loader 条目（moduleName/enabled/fiberPhase）；静态插件的增删/启停不在本次范围（cordis.yml 写回会 bake patch 行，改动走「源文件」tab）。",
...
"config.emptyExtensions": "暂无已安装扩展。",
  ...
  "settings.dsh.statusTitle": "DSH 后端状态",
  "settings.dsh.statusDesc": "DSH_HOME：{home}",
  "settings.dsh.statusDescUnknown": "DSH_HOME 未确定",
  "settings.dsh.started": "已启动",
  "settings.dsh.notStarted": "未启动（首次使用 DSH 会话时自动启动）",
  "settings.dsh.homeDir": "DSH 数据目录（DSH_HOME）",
  "settings.dsh.homeDirDesc": "默认使用 ~/.dsh（与 dsh CLI 共用配置/会话）；留空回退自动。切换后重启 host 生效，模型/预设/插件等完整配置见配置页 DSH 分区。",
  "settings.dsh.homeDirPlaceholder": "留空 = 自动（~/.dsh 优先）",
