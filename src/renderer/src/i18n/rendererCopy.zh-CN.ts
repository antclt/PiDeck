...
export const zhCN = {
  ...
  "config.dsh.foreignSessionsHint":
    "由 dsh-web 等其他工具在 DSH_HOME 里创建的会话，host 启动后会自动导入侧栏（目标项目按目录自动匹配；没有目录的进入「外部会话」项目）。此处仅列出尚未映射的会话，可单独导入或一键全部导入；导入后从本清单消失，不会重复导入。",
  "config.dsh.importSession": "导入",
  "config.dsh.imported": "已导入外部 DSH 会话。",
  "config.dsh.syncAll": "全部导入",
  "config.dsh.synced": "已导入 {count} 个外部会话。",
...
"config.emptyExtensions": "暂无已安装扩展。",
  ...
  "settings.dsh.autoImportForeignDesc": "host 启动后把 dsh-web 等其他工具在 DSH_HOME 创建的会话自动导入侧栏（按目录匹配项目，无目录进入「外部会话」项目；已导入的跳过）。关闭后需在配置页手动导入。",
