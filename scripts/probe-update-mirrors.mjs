...
const MIRRORS = [
	{ id: "ghfast", base: "https://ghfast.top/https://github.com/pideck-app/PiDeck/releases/latest/download" },
	{ id: "ghproxy-net", base: "https://ghproxy.net/https://github.com/pideck-app/PiDeck/releases/latest/download" },
	{ id: "ghproxy-cxkpro", base: "https://ghproxy.cxkpro.top/https://github.com/pideck-app/PiDeck/releases/latest/download" },
];
...
function fetchText(url, outFile = "/tmp/probe-text.bin") {
	const res = spawnSync("curl", ["-sL", "--noproxy", "*", "-x", "", "--max-time", "30", "-o", outFile, "-w", "%{http_code}|%{time_total}", url], { encoding: "utf8", maxBuffer: 1 << 20 });
	if (res.error) return { error: res.error.message };
...
for (const mirror of only) {
	...
	console.log(p1.error ? `✖ 下载预检失败：${p1.error}` : `· Range 0-1MB（跟随 302）→ HTTP ${p1.code}（期望 206${p1.code === 206 ? " ✅" : "/200 全量说明丢 Range ❌"}）| ${(p1.size / 1024).toFixed(0)}KB | ${p1.timeSec}s | ${p1.speedKBps}KB/s`);

	const p2 = fetchBin(`${mirror.base}/${SETUP}`, { range: "69200000-70200000", outFile: "/tmp/probe-part2.bin" });
	console.log(p2.error ? `✖ 中段分片失败：${p2.error}` : `· Range 66-67MB（中段，跟随 302）→ HTTP ${p2.code}（期望 206${p2.code === 206 ? " ✅" : "/200 全量说明丢 Range ❌"}）| ${(p2.size / 1024).toFixed(0)}KB | ${p2.timeSec}s | ${p2.speedKBps}KB/s`);
	...
	if (FULL) {
		...
		console.log(`· 完整下载 ${(full.size / 1024 / 1024).toFixed(1)}MB → ${full.timeSec}s（${full.speedKBps}KB/s）| sha512 校验 → ${expected && actual === expected ? "✅ 通过" : `❌ 不匹配（期望 ${expected.slice(0, 16)}…，实际 ${actual.slice(0, 16)}…）`}`);
	}
}
