...
| **S5 功能补齐 P2 + 增强** | ✅ F6 imagegen 对 DSH 隐藏、F7 署名 i18n、E14 getStatus 语义、D16 restart 校验、E12 桥 origin 校验、D13 setThinking 顺序、G1 删除提示、E16 getClient 单例、F9 路径拼接、G3/D11 孤儿检测（commit f183f548/2062b278/c7fe6112）；D14/D15/E13 论证为现状可接受或已知限制（投影消息变更事件低频、batch 损坏已按拒绝处理、host 格式解析为双份代码已注释）；⏳ G2 图片附件（桥字节载荷，大）、G14 归档（大）、G17 rpc log（中）、G13 插件管理 UI（大） | S4 | 全量手测 + 打包验证 | ⏳ 10/14 |
