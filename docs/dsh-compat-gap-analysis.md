...
| **S4 功能补齐 P1** | ✅ F5 复制路径入口、G11 设置页 DSH tab、G12 plan/权限徽标、G4 `/commands` 建议菜单、G16 usage 统计投影、D7 注释对齐（C9 顺带）、D8 abort 后 error 不投影、D9 工具卡收口（commit 976c02c8 / b2b2a212）；⏳ G9 会话搜索（主进程 session.search 接入 + 侧栏搜索 UI）、G5/G6 goals/subagents UI（需新 IPC + 时间线呈现） | S3 | e2e 新增覆盖 | ⏳ 3/10 |
