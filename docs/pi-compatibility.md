...
| 新增模型/provider/thinking 能力 | 运行中的 Pi CLI 自动获得；配置自适应模板只读 PiDeck 自带 bundled `pi-ai` catalog（当前 `0.82.1`），endpoint `/models` 实报字段优先合并，不读 capability cache / 外部 Pi 目录 | 已完成 | `src/main/pi/modelCapabilityResolver.ts`、`src/renderer/src/utils/modelSpecAutoFill.ts`、`ConfigModal.handleResetModelToAdaptive`、`tests/adaptiveModelTemplate.test.mjs` | 升级 bundled `pi-ai` 后跑 typecheck、catalog 与自适应模板测试，并确认 DSH 不会被拆成另一份 pi-ai |
...
- [ ] 评估升级 `@earendil-works/pi-ai` 到 `0.84.3`；不要只升级根依赖，先等/推动 DSH adapter 对齐版本。
...
| PiDeck 主进程 | 直接读取 `@earendil-works/pi-ai/dist/providers/data/*.json` | `package.json` 为 `^0.82.1`，锁定 `0.82.1`；只补全 context/maxTokens/reasoning/images/name，不实例化 SDK，也不读取 catalog 的 `thinkingLevelMap` |
| DSH host | `dsh-llm-pi-ai` 动态调用 `createModels`、catalog、`getSupportedThinkingLevels` 和 provider API | 当前依赖图实际为 `@earendil-works/pi-ai@0.82.1`；`dsh-llm-pi-ai` 声明 `^0.82.1`，对 0.x semver 等价于 `>=0.82.1 <0.83.0`，不包含 `0.84.3` |

因此：

- **仅升级 PiDeck 根 `@earendil-works/pi-ai` 到 0.84.3，不会让 Pi 后端 runtime 变成 0.84.3**，也不会自动打开 Pi UI 的按模型过滤；
- 如果 npm 重新解析，DSH adapter 可能在自己的依赖树中继续安装 `0.82.1`，形成 PiDeck catalog 0.84.3、DSH runtime 0.82.1 的双版本；
- 不建议用 `overrides` 强行把 DSH 的 pi-ai 改成 0.84.3。应等待/推动 `dsh-llm-pi-ai` 发布声明兼容 0.84.x 后，再整体升级 DSH 相关包并做 host smoke、thinking effort、流式请求和 provider catalog 回归。

### 是否打包进 PiDeck

**是，PiDeck 自己的 `@earendil-works/pi-ai@0.82.1` 会进入安装包**：

- 它位于 `dependencies`，不是 `devDependencies`；
- electron-builder 默认把生产依赖放入 `resources/app.asar/node_modules`；
- `src/main/pi/piAiBuiltinCatalog.ts` 运行时从包内读取 `dist/providers/data/*.json`；
- 已检查现有 `release/win-unpacked/resources/app.asar`：包含 `@earendil-works/pi-ai`、`@deepseek-ai/dsh-llm-pi-ai` 及 38 个 catalog JSON；
- `after-pack-cleanup.js` 只清理文档、source map、类型声明和测试文件，不删除 pi-ai 的运行时代码或 catalog JSON。

但这份打包的 pi-ai **不是 Pi 后端 runtime 使用的那一份**。PiDeck 通过 `PiLocator` 执行用户已经安装的 `pi` CLI；Pi CLI 自己携带/解析自己的 pi-ai。PiDeck 不打包 `pi-coding-agent`，也不把主进程里的 pi-ai 版本注入外部 Pi 进程。

升级记录：只有当 `@deepseek-ai/dsh-llm-pi-ai` 发布明确兼容 `@earendil-works/pi-ai 0.84.x` 的版本后，才整体升级 PiDeck 根依赖、DSH adapter 和 lockfile；完成 typecheck、catalog/迁移测试、DSH host smoke、thinking effort、流式请求和 `npm run pack` 后再关闭本项。
