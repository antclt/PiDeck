...
| 新增模型/provider/thinking 能力 | 运行中的 Pi CLI 自动获得；配置自适应模板只读 PiDeck 自带 bundled `pi-ai` catalog（当前 `0.84.4`），endpoint `/models` 实报字段优先合并，不读 capability cache / 外部 Pi 目录 | 已完成 | `src/main/pi/modelCapabilityResolver.ts`、`src/renderer/src/utils/modelSpecAutoFill.ts`、`ConfigModal.handleResetModelToAdaptive`、`tests/adaptiveModelTemplate.test.mjs` | 升级 bundled `pi-ai` 后跑 typecheck、catalog 与自适应模板测试，并确认 DSH 仍使用 adapter 声明兼容的独立版本 |
...
- [x] PiDeck 主进程 bundled `@earendil-works/pi-ai` 已升级到 `0.84.4`；DSH adapter 继续使用其声明兼容的 `0.82.1`，不通过 overrides 强行升级。
...
| PiDeck 主进程 | 直接读取 `@earendil-works/pi-ai/dist/providers/data/*.json` | `package.json` 为 `^0.84.4`，lock 锁定 `0.84.4`；补全 context/maxTokens/reasoning/input/name/thinkingLevelMap，不实例化 SDK |
...
- **仅升级 PiDeck 根 `@earendil-works/pi-ai` 到 0.84.4，不会让 Pi 后端 runtime 变成 0.84.4**，也不会自动打开 Pi UI 的按模型过滤；
- DSH adapter 在自己的依赖树中解析 `0.82.1`，因此 PiDeck catalog `0.84.4` 与 DSH runtime `0.82.1` 会有意共存；
...
**是，PiDeck 主进程使用的 `@earendil-works/pi-ai@0.84.4` 会进入安装包**：
...
- `scripts/verify-asar-runtime.js`、`scripts/check-dsh-asar.mjs` 与 `scripts/check-dsh-boot.mjs` 分别守护 app 与 DSH runtime 所需的 pi-ai 运行时文件；
...
升级记录：PiDeck 主进程 catalog 已单独升级到 `0.84.4`；DSH 仍保留 adapter 兼容的嵌套 `0.82.1`。只有当 `@deepseek-ai/dsh-llm-pi-ai` 发布明确兼容 `@earendil-works/pi-ai 0.84.x` 的版本后，才升级 DSH adapter/runtime 依赖树，并完成 typecheck、catalog/迁移测试、DSH host smoke、thinking effort、流式请求和 provider catalog 回归。
